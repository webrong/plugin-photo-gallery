<template>
  <VModal ref="modal" mount-to-body :title="isEdit ? '编辑相册' : '新建相册'" :width="700" @close="emit('close')">
    <div class="space-y-4 md:grid md:grid-cols-4 md:gap-6">
      <div class="md:col-span-1">
        <span class="text-base font-medium text-gray-900">基本信息</span>
      </div>
      <div class="space-y-4 md:col-span-3">
        <div v-if="albumGroups.length > 0">
          <label class="form-label">所属分组</label>
          <select v-model="form.spec.groupName" class="form-input">
            <option value="">无分组</option>
            <option v-for="g in albumGroups" :key="g.metadata.name" :value="g.metadata.name">
              {{ g.spec.displayName }}
            </option>
          </select>
        </div>
...
const props = withDefaults(defineProps<{ album?: Album }>(), { album: undefined });
...
function createForm(): Album {
  return {
    ...
    kind: "Album",
    metadata: { name: "", generateName: "album-" },
    spec: { displayName: "", slug: "", description: "", cover: "", priority: 0, visible: true, groupName: "" },
  };
}

const form = ref<Album>(createForm());
...
onMounted(() => {
  fetchGroups();
  if (props.album) {
    form.value = JSON.parse(JSON.stringify(props.album));
  }
});
</script>
